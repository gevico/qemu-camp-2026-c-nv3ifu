...
int check_add_overflow_asm(unsigned int a, unsigned int b) {
    unsigned char carry;
    __asm__ volatile(
        "addl %2, %1; setc %0" : "=qm"(carry), "+r"(a) : "r"(b) : "cc"
    );
    return carry;
}
...
int check_sub_overflow_asm(unsigned int a, unsigned int b) {
    unsigned char carry;
    __asm__ volatile(
        "subl %2, %1; setc %0" : "=qm"(carry), "+r"(a) : "r"(b) : "cc"
    );
    return carry;
}
...
int check_mul_overflow_asm(unsigned int a, unsigned int b) {
    ...
    __asm__ volatile(
        "mull %3; setc %0" : "=qm"(overflow), "=d"(high_bits), "+a"(a) : "r"(b) : "cc"
    );
...
int check_div_overflow_asm(unsigned int a, unsigned int b) {
    ...
    __asm__ volatile(
        "testl %1, %1; setz %0" : "=qm"(is_div_zero) : "r"(b) : "cc"
    );
    return is_div_zero;
}
