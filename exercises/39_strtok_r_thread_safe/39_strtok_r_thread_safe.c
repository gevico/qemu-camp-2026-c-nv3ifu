...
static int is_delim(char c, const char *delim) {
    if (!delim) return 0; while (*delim) if (c == *delim++) return 1; return 0;
}

/* 线程安全版本：通过 saveptr 维护调用状态，不使用静态变量 */
char *strtok_r(char *str, const char *delim, char **saveptr) {
    if (!delim || !saveptr) return NULL; char *p = str ? str : *saveptr; if (!p) return NULL; while (*p && is_delim(*p, delim)) p++; if (!*p) { *saveptr = p; return NULL; } char *token = p; while (*p && !is_delim(*p, delim)) p++; if (*p) *p++ = '\0'; *saveptr = p; return token;
}
