...
void register_event(enum EVENT_TYPE type, void (*callback)(void*), void* arg) {
    if (type < 0 || type >= EVENT_MAX) return; g_callbacks[type] = callback; g_callback_args[type] = arg;
}
...
void trigger_event(enum EVENT_TYPE type) {
    if (type < 0 || type >= EVENT_MAX || !g_callbacks[type]) return; g_callbacks[type](g_callback_args[type]);
}
