 ...
 */
typedef struct {
    uint16_t version_be; uint16_t length_be; uint8_t flags_raw;
} proto_header_raw_t;
 ...
 */
typedef struct {
    unsigned ver_minor : 4; unsigned ver_major : 4; unsigned length : 16; unsigned flags : 5; unsigned reserved : 3;
} proto_header_bits_t;
...
int main(void) {
    ...
    uint16_t version = be16_to_cpu(raw.version_be); unsigned ver_major = (version >> 4) & 0x0f; unsigned ver_minor = version & 0x0f;
    ...
    proto_header_bits_t view = {.ver_major = ver_major, .ver_minor = ver_minor, .length = length, .flags = flags, .reserved = raw.flags_raw >> 5};
